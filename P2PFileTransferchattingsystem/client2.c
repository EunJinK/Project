...
int main(int argc, char *argv[])
{
	...
	FILE *fp, *eunjin2;
	char c,fname[3][20] = {"FILE1_2", "FILE2_2","FILE3_2"} ;
	...
	while(1)
	{
		if ( pid > 0)
		{
			...
			send(sockfd, message2, strlen(message2)+1,0);
			...
			if(!(strstr(message2,file)==NULL))
			{
				...
				if(pid1 == 0)
				{
					...
					if(fsendfd == -1)
					{
						perror("\nUser2 Server-socker() error lol!");
						exit(1);
					}
					else printf("User2 Server-socker() fsendfd is OK...\n");
					...
					{
						perror("User2 Server-bind() error lol!");
						exit(1);
					}	
					else printf("User2 Server-bind() is OK...\n");
	
					if(listen(fsendfd, BACKLOG) == -1)
					{
						perror("User2 listen() error lol!");
						exit(1);
					}
					else printf("User2 listen() is OK...\n");
					...
					for(int h =0 ;h<3;h++)
					{
						recv(new_fd, User1fname[h], sizeof(User1fname[h]),0);
					}
					printf("\n-------FILE MODE START-------\n");
					printf("-------FILE LIST-------\n");
					for(int f = 0; f<3;f++)
		...
		{
			rcv_byte = recv(sockfd, message1, sizeof(message1),0);
			if(strlen(message1)>1 && rcv_byte != -1)	
			{
				if(!(strstr(message1,file)==NULL))
				{
					...
					if(pid1 == 0)
					{
						...
						else printf("User2 Client-socker() frecvfd is OK...\n");
						...
						{
							perror("User2 Client-connect() error lol\n\n");
							exit(1);
						}
						else printf("User2 Client-connect() is OK...\n\n");

						close (sockfd);

						for(int h=0;h<3;h++)
						{
							eunjin2 = fopen(fname[h],"w");
							fprintf(eunjin2, "Num %d File >> Today 12/%d\n",h+1,h+14);
							sleep(1);
							send(frecvfd, fname[h],strlen(fname[h])+1,0);
							fclose(eunjin2);
						}
						while((c=fgetc(eunjin2))!=EOF)
						{
							fputc(c,stdout);
						}
						...
						if(num == 1)
						{
							...
							eunjin2 = fopen(fname[0],"r");
							if(eunjin2 == NULL)
								perror("Error opening file\n");
							else
							{
								fgets(filemessage,100,eunjin2);
								sleep(1);
